var pokemonList, fillGeneratedTable = function() {
    ...
    for (var i = 0; i < 6; i++)
    {
        $('.pokemon' + (i + 1)).append('<a href="/' + pokemonList[i].name.replace(": ", "_").replace(' ', '_').toLowerCase() + '/" target="_blank"><img title="' + pokemonList[i].name.replace('_', ' ') + ' (Click to learn more)" src="/images/pokemon/' + pokemonList[i].id + '.png")" /></a>');
    }
};
