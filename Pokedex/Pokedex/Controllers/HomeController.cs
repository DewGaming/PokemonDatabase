...
namespace Pokedex.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [AllowAnonymous]
        [Route("pokemon/{pokemonName}/locations/{pokemonId:int}/{generationId:int}")]
        ...
        public IActionResult PokemonLocations(string name)
        {
            ...
            {
                List<PokemonViewModel> pokemonList = new List<PokemonViewModel>();
                PokemonViewModel pokemonDetails = this.dataService.GetPokemonLocationDetails(pokemon, null, this.appConfig);
                pokemonDetails.SurroundingPokemon = this.dataService.GetSurroundingPokemon(pokemon.Id);
                ...
                if (altForms.Count > 0)
                {
                    ...
                    foreach (var p in altForms)
                    {
                        ...
                        {
                            form = this.dataService.GetFormByAltFormId(p.Id);
                            pokemonDetails = this.dataService.GetPokemonLocationDetails(p, form, this.appConfig);
